...
def read_friendship_network(filename):
    ...
    # tallennetaan muuttuja network johon tallennetaan kaverisuhteet
    # tämän sanakirjan avulla myös tehdään kaikki eri funktioiden komennot
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    # tarkistetaan, että ketään ei voida lisätä hänen omana kaverinaan
    if name1 == name2:
        return None

    # Lisätään networkkiin tieto kaverisuhteesta nimien välillä.
    # Tarkistetaan myös, että nimi ei ole valmiiksi listassa.
    if name1 not in network:
        network[name1] = []
    if name2 not in network[name1]:
        network[name1].append(name2)

    # Tehtävänannon mukaan toistetaan komento sillä yksi kutsu lisää molempiin suuntiin.
...
def print_command(network, namelist):
    ...
    # Tehtävänannon mukaan tulostetaan yläotsikoksi avain eli nimi aakkosjärjestyksessä.
    for key in sorted(network):
        print(key)

        # Jokaisen otsikon alle tulostetaan ranskalaisilla viivoilla nimen ystävät.
...
def friends_command(network, namelist):
    ...
    # Tarkistetaan, että nimi on listassa, jos nimeä ei löydy listasta
    # Tulostetaan Error viesti ja jatketaan ohjelman toimintaa.
    if name not in network:
        print(f"Error: '{name}' is an unknown name.")
        return

    # Jos nimi onkin listassa niin tulostetaan sen sisältämät arvot
    # eli kaverisuhteet aakkosjärjestyksessä.
    if name in network:
...
def common_friends_command(network, namelist):
    ...
    # Tarkistetaan, että nimi on sanakirjassa.
    # Jos ei ole tulostetaan Error viesti ja palataan alkuun.
    for name in namelist:
        if name not in network:
            print(f"Error: '{name}' is an unknown name.")
            return
    ...
    # Tarkistetaan, että nimet eivät ole samat.
    # Jos yritetään lisätä itseään itsensä kaveriksi annetaan Error viesti ja palataan.
    if name1 == name2:
        print(f"{name1} has no common friends with him/herself.")
        return

    # Muuten etsitään sanakirjasta molemmilla avaimilla yhtenäiset arvot ja lisätään ne listaan.
    yhteiset = []
    if name1 != name2:
        for value1 in network[name1]:
            for value2 in network[name2]:
                if value1 == value2:
                    yhteiset.append(value1)

    # Tarkistetaan, että yhteisiä kavereita löytyi.
    # Jos ei löytynyt tulostetaan siitä viesti.
    # Jos löytyi tulostetaan ne aakkosjärjestyksessä tehtävänannon mukaan.
    if len(yhteiset) == 0:
        print(f"{name1} and {name2} have no common friends.")
    else:
        for value in sorted(yhteiset):
            print(value)
